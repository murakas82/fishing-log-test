...
(function () {
  ...
  function applyTheme(theme = getTheme()) {
    const t = (theme && ALLOWED_THEMES.has(theme)) ? theme : "dark";
    // IMPORTANT: apply to both <html> and <body> so top background changes too
    document.documentElement.setAttribute("data-theme", t);
    document.body.setAttribute("data-theme", t);
  }

  // Translation dictionary (MERGED: translations1.js + theme keys)
  const TRANSLATIONS = {
    en: {
      // Home / index.html
  ...
  function applyTranslations() {
    ...
    document.querySelectorAll("[data-i18n]").forEach((el) => {
      ...
      // Support newline strings (used in legendDescription)
      if (value.indexOf("\n") >= 0) {
        el.innerHTML = "";
        value.split("\n").forEach((line, idx) => {
    ...
    // Optional: style language buttons if some pages use .lang-active
  ...
  // Apply saved theme ASAP (avoids flash)
